...
namespace GameLibrary.Scripts.Character
{
    public class ZombieMovementScript : Script
    {
        ...
        public ZombieMovementScript(Game3DObject target, Animation animation, float speed = 20f, float radius = 70f, int damage = 10, float cooldown = 0.2f)
        {
            _target = target;
            _radius = radius;
            _movement = new CharacterMovement(animation, speed);
            _damage = damage;
